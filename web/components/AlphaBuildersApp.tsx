...
import { ArrowRight,ArrowUpRight,Check,Clipboard,Code2,Github,Network,ShieldCheck,Sparkles,Wallet,X } from "lucide-react";
...
<div className="hero-copy"><div className="pilot-badge"><Sparkles size={15}/> Programa piloto · Base Sepolia</div><h1>Aprenda construindo. <span>Prove contribuindo.</span></h1><p className="hero-lead">Um laboratório open source para transformar pequenas contribuições públicas em evidências técnicas verificáveis — sem venda de token, sem promessa financeira e sem atalhos de portfólio.</p><div className="hero-actions"><a className="neo-button primary" href="#challenge">Ver desafios <ArrowRight size={18}/></a><a className="neo-button secondary" href="#flow"><Code2 size={18}/> Como funciona</a></div><div className="hero-footnote"><ShieldCheck size={18}/> Ambiente experimental de testnet. Nenhuma compra, transferência, assinatura ou taxa é solicitada.</div></div>
...
<section className="purpose-panel" aria-labelledby="purpose-title"><div className="purpose-heading"><ShieldCheck size={28} aria-hidden="true"/><div><h2 id="purpose-title">Propósito do ALPHA Builders</h2><p>Fortalecer portfólios com código aberto real, rastreável e verificável. Aqui, cada contribuição deixa um rastro técnico que fala por você.</p></div></div><div className="story-band" aria-label="Princípios do piloto"><div><Github size={26} aria-hidden="true"/><strong>Aberto</strong><small>Tudo é público e auditável.</small></div><div><ShieldCheck size={26} aria-hidden="true"/><strong>Verificável</strong><small>Evidências técnicas on-chain e off-chain.</small></div><div><Code2 size={26} aria-hidden="true"/><strong>Sem token à venda</strong><small>Sem venda, sem rendimento e sem promessa financeira.</small></div><div><Network size={26} aria-hidden="true"/><strong>Para builders</strong><small>Feito para transformar contribuição em prova técnica.</small></div></div></section>
<section className="challenge-latest section-shell" id="challenge"><div className="section-kicker">UTILIDADE 01 / DESAFIO PILOTO</div><div className="challenge-grid"><div className="challenge-copy"><h2>Revisão estruturada de <span>README.</span></h2><p>Escolha um projeto público, identifique um ponto confuso e proponha uma melhoria objetiva na documentação. Sua contribuição permanece pública e verificável no GitHub.</p><div className="challenge-meta"><div><small>ENTREGA</small><strong>Melhoria pública e rastreável</strong></div><div><small>RECONHECIMENTO</small><strong>Experimental, sujeito à validação</strong></div></div><a className="text-link" href={repositoryUrl} target="_blank" rel="noreferrer">Abrir repositório <ArrowUpRight size={17}/></a></div><div className="task-panel"><span className="panel-index">DESAFIO / 01</span><h3>O que uma boa contribuição deve melhorar?</h3><ul><li><Check size={17}/> Contexto e objetivo do projeto</li><li><Check size={17}/> Instalação reproduzível</li><li><Check size={17}/> Stack e decisões técnicas</li><li><Check size={17}/> Testes e validações</li><li><Check size={17}/> Demonstração quando fizer sentido</li></ul><div className="safety-note"><ShieldCheck size={19}/> Sem assinatura, transferência, approve, swap, bridge ou staking.</div></div></div></section>
<section className="flow-section section-shell" id="flow"><div className="section-heading"><div className="section-kicker">COMO FUNCIONA</div><h2>Da primeira conexão à sua prova pública.</h2></div><div className="flow-grid">{steps.map(([index,title,body])=><article className="flow-card" key={index}><span>{index}</span><h3>{title}</h3><p>{body}</p></article>)}</div></section>
...
<SubmissionSection account={account} onCorrectNetwork={onCorrectNetwork} connect={connect} exampleBuilderIssue={exampleBuilder?.issue}/>
